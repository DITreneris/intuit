...
- **Proof — valdymo diagrama (HeroSystemDiagram v2):** [`HeroSystemDiagram.astro`](src/components/HeroSystemDiagram.astro) — platesnis ir vizualiai akcentuotas 2 žingsnis („Žinutė“ / narrative), horizontalioji bazinė linija su subtilia juostinės animacijos (`stroke-dashoffset`; išjungiama esant `prefers-reduced-motion`), storesnės jungtys ir didesni rodyklių markeriai, sutrumpinta punktyrinė feedback kilpa, išorinis konteineris (`bg-paper`, silpnesnis `border`, `ring`); suvienodintos ikonos (vienodas stroke, 28×28 dėžės). i18n [`src/i18n/lt.ts`](src/i18n/lt.ts) / [`src/i18n/en.ts`](src/i18n/en.ts) — outcome-first `systemStrip`, trumpesnis `diagramCaption`, atnaujinti `nodeHint1–4`, naujas `diagramLoopLabel`, atnaujintas `svgDesc`; `aria-describedby` įtraukia kilpos etiketę.
- **Landing UX (mobilus + skaitomumas):** [`Hero.astro`](src/components/sections/Hero.astro) — natūralus stulpelių eiliškumas (tekstas viršuje `< md`); [`TrustedBy.astro`](src/components/sections/TrustedBy.astro) + i18n `trustedBy.hint` — aiškus slinkties paaiškinimas, `text-xs` hintui; [`ChaosVsSystem.astro`](src/components/sections/ChaosVsSystem.astro) + `chaosVsSystem.sectionTitle` (LT/EN) — matoma H2; [`Process.astro`](src/components/sections/Process.astro) — vertikali `border-l` jungtis mobiliajame (`md+` be pakeitimo); [`HeroSystemDiagram.astro`](src/components/HeroSystemDiagram.astro) — antraštės/caption `text-xs` mobilėje, didesni šriftai tik mobiliajame SVG; [`Header.astro`](src/components/Header.astro) — primary CTA pirmoje mobilaus meniu eilėje.
- **Logotipas (wordmark) ir brand failai:** kanoniniai JPG iš `INTUIT LOGO/jpg/` nukopijuoti į [`public/brand/`](public/brand/) (`logo-light.jpg`, `logo-dark.jpg`, `logo-pattern-light.jpg`, atnaujintas `logo-pattern-inverse.jpg`) — sutampa su OG paveikslu ([`Layout.astro`](src/layouts/Layout.astro) `og:image` / `twitter:image`) ir kontaktų bloku (`logo-dark.jpg`). [`Header.astro`](src/components/Header.astro) ir [`TrustedBy.astro`](src/components/sections/TrustedBy.astro) — **tik** `logo-light.jpg` (be `<picture>` / monogramos `logo-light.svg`), logotipo nuoroda su `shrink-0` (ne `min-w-0`); [`docs/BRAND.md`](docs/BRAND.md), [`docs/DESIGN-SYSTEM.md`](docs/DESIGN-SYSTEM.md) — inline wordmark strategija.
