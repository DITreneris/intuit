...
- **Nav — premium juosta (kompaktiškos etiketės):** [`src/components/Header.astro`](src/components/Header.astro) — penkios anchor nuorodos (be `#about`). i18n [`src/i18n/lt.ts`](src/i18n/lt.ts): `nav.solution` **Metodas**, `nav.proof` **Rodikliai**, `nav.offers` **Planai** (nepakitę); [`src/i18n/en.ts`](src/i18n/en.ts): **Method**, **Metrics**, **Plans**. [docs/COPY-GLOSSARY.md](docs/COPY-GLOSSARY.md) §6.
