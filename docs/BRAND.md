...
- **Web spalvos ir diagramos:** viena pagrindinė **brand** mėlyna (`accent`); `diagramElectric` sutampa su ja; `bg-hero-premium` pabaiga — labai silpnas tos pačios mėlynos tint (be atskiros indigo); antriniai tokenai (`diagramLoop`, `diagramStrokeAi`) lieka švelnūs.
