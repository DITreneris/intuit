...
- **Proof — valdymo diagrama (HeroSystemDiagram v3):** [`src/components/HeroSystemDiagram.astro`](src/components/HeroSystemDiagram.astro) — vientisa **spine** (desktop: `M 78 68 L 534 68` po kortelėms; mobile: vertikali ašis), subtili juostinė animacija (`hero-diagram-flow-spine` / `hero-diagram-flow-spine-m`, išjungiama su `prefers-reduced-motion`); pašalinti atskiri connector segmentai ir **SVG feedback kilpa**; iteracija — caption + `diagramLoopLabel` (dekoratyvus ↺). Ikonos fiksuoto **24×24** ritmo, **stroke 1.5**, be antro rėmo aplink ikoną; 2 žingsnio **tint** (`accent`); išorė — `border` be `shadow-soft`/`ring` sluoksniavimo. Copy: [`src/i18n/lt.ts`](src/i18n/lt.ts) / [`src/i18n/en.ts`](src/i18n/en.ts) `hero.diagram` — `systemStrip`, `svgDesc`. Dizaino doc: [`docs/DESIGN-SYSTEM.md`](docs/DESIGN-SYSTEM.md) (skyrius „Hero diagrama“).
