...
- **Kontaktai — Prompt Anatomy attribution:** [`src/components/sections/FinalCta.astro`](src/components/sections/FinalCta.astro) — po kontaktų bloko (`#contact`) nuoroda **„Made with PA“** į `https://www.promptanatomy.app` (`target="_blank"`); i18n [`src/i18n/lt.ts`](src/i18n/lt.ts) / [`src/i18n/en.ts`](src/i18n/en.ts) — `finalCta.madeWithPaLabel` (LT ir EN ta pati angliška eilutė).
...
- **Nav — premium juosta + „Apie“ į footerį:** [`src/components/Header.astro`](src/components/Header.astro) — penkios anchor nuorodos (be `#about`); [`src/components/Footer.astro`](src/components/Footer.astro) — antrinė `<nav>` su `nav.about` ir `nav.contact`. i18n [`src/i18n/lt.ts`](src/i18n/lt.ts): `nav.solution` **Metodas**, `nav.proof` **Rodikliai**, `nav.offers` **Planai** (nepakitę); [`src/i18n/en.ts`](src/i18n/en.ts): **Method**, **Metrics**, **Plans**. [docs/COPY-GLOSSARY.md](docs/COPY-GLOSSARY.md) §6.
